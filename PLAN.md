...
**262/269 Nix tests passing** (97.4%) — selected tests from the upstream Perl test suite. (35 of the 304 listed entries reference test paths that don't exist in the unpacked perl-5.42.0 tarball, so they are skipped at our local-runner level.) Sandbox-style survey (using full-path reference perl invocation matching the Nix sandbox's `${pkgs.perl}/bin/perl` form) confirms the 7 remaining byte-for-byte diff sources: `opbasic/concat` (15 lines — byte-mode string `eq`/`.=` and `tie`+multiconcat eval order), `op/print` (19 lines — `:utf8` IO layer overlong-byte preservation), `op/length` (25 lines — `use bytes; length` on strings whose chars share byte values with their UTF-8 encoding), `op/pos` (77 lines — defelem PVLV machinery for `=~ //g` propagation through alias slots), `op/undef` (122 lines — most remaining diff is hash-iteration order during `undef %h` differing between Rust HashMap and Perl, plus Devel::Peek::Dump output for the SV/PV-allocation tests), `base/lex` (125 lines — heredocs interleaved with re-eval, q-comment spans, vstring labels, `${...}` literal-triple-dot etc.), and `re/regexp` (1674 lines — bulk regex-engine feature gap covering ~470 distinct upstream tests; capture-group-reset semantics and conditional regex `(?(N)…)` are the largest remaining clusters). Tests now passing that were previously listed as failing: `op/array`, `op/heredoc`, `op/repeat`, `op/join`, `op/chr`, `opbasic/cmp`, `op/eval`.
