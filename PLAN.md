...
- **DESTROY in a closure sees the LIVE caller scope, not its captured snapshot** — `enter_named_sub_scope` was stashing the dynamic frames for closure-bearing named subs so they'd see only the captured-at-definition lexicals. That was wrong for `Class::DESTROY`: when the destructor fires synchronously during `undef %h` (one DESTROY per popped key), the handler must see the partially-shrunk LIVE container. Special-case names ending in `::DESTROY` — leave the dynamic frames un-stashed so `keys %h` etc. inside the destructor walk through the live caller scope. op/undef diff: 199 → 134 lines (-65, largest remaining diff is hash-iteration ordering between Rust HashMap and Perl, plus Devel::Peek::Dump output).
