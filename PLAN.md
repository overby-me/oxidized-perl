...
- **`next::method` and `next::can` via `mro::_nextcan`** — adding the `mro::_nextcan(obj, wantcan)` core builtin makes mro.pm's `next::method` / `next::can` / `maybe::next::method` shims work end-to-end. Coupled with three supporting changes: (1) `package next;` (and `last`/`redo`) parses as a regular package declaration even though those names tokenise as loop-control keywords; (2) `$obj->Pkg::method` dispatches `Pkg::method` directly when the qualified name exists, bypassing the @ISA walk — required for `$self->next::method(@args)` to reach the literal sub from mro.pm; (3) `goto &$var` accepts a scalar-deref target (parser encodes as `&$NAME`; runtime resolves through `get_var` for a `Value::CodeRef`). 50 of 73 mro/* tests now pass (up from 15 before this iteration's mro work).
