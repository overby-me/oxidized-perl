...
**185/191 broader Nix tests passing** (96.9%) — selected tests from the upstream Perl test suite. (35 of the 304 listed entries reference test paths that don't exist in the unpacked perl-5.42.0 tarball, so they are skipped at our local-runner level.) Remaining byte-for-byte diff sources: `op/pos` (7 lines — nested `for my $one(pos $x)` aliasing semantics), `base/lex` (12 lines — `\xDF` non-ASCII identifier start under `use utf8`, `s/${foo#}//e` regex-comment-in-interp), `opbasic/concat` (15 lines — byte-mode string `eq`/`.=` and `tie`+multiconcat eval order), `op/print` (19 lines — `:utf8` IO layer overlong-byte preservation), `op/length` (25 lines — `use bytes; length` on strings whose chars share byte values with their UTF-8 encoding), and `op/undef` (~125 lines — hash-iteration order during `undef %h` differing between Rust HashMap and Perl, plus Devel::Peek::Dump output for the SV/PV-allocation tests). All remaining failures require architectural features (per-string UTF-8 flag, Perl's exact hash function, foreach PVLV aliasing) that span the value system + lexer + interpreter together.
