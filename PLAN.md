...
**262/269 Nix tests passing** (97.4%) — selected tests from the upstream Perl test suite. (35 of the 304 listed entries reference test paths that don't exist in the unpacked perl-5.42.0 tarball, so they are skipped at our local-runner level.) Remaining byte-for-byte diff sources: `op/hashwarn` (7 lines — `$h{\1}` interpolated hash key with take-ref evaluates the escape too early), `op/pos` (7 lines — nested `for my $one(pos $x)` aliasing semantics), `base/lex` (12 lines — `\xDF` non-ASCII identifier start, `s/${foo#}//e` regex-comment-in-interp), `opbasic/concat` (15 lines — byte-mode string `eq`/`.=` and `tie`+multiconcat eval order), `op/sub_lval` (19 lines — `Internals::SvREADONLY $x, 1` for scalars), `op/print` (19 lines — `:utf8` IO layer overlong-byte preservation), `op/length` (25 lines — `use bytes; length` on strings whose chars share byte values with their UTF-8 encoding), `op/state` (34 lines — `:shared` accepted at parse time but reference perl errors, leaving us with more passing tests than ref expects), `op/undef` (~120 lines — hash-iteration order during `undef %h` differing between Rust HashMap and Perl, plus Devel::Peek::Dump output for the SV/PV-allocation tests), and `re/regexp` (539 lines — bulk regex-engine feature gap; recursive groups, capture-group-reset semantics, and conditional regex `(?(N)…)` are the largest remaining clusters).
