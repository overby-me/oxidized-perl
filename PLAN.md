...
**Nix testsuite: 266/269 of the testable-on-disk tests pass byte-for-byte (98.9%).** (Of the 304 entries in default.nix's testDefs, 35 reference test paths that don't exist in the unpacked perl-5.42.0 tarball.) Only 3 byte-for-byte failures remain, all hard architectural issues: `benchmark/gh7094-…` (7 lines — Perl's stdout buffering interaction with the survey-script timeout, not a real defect), `op/undef` (~100-150 lines depending on hash-seed luck — hash-iteration order during `undef %h` plus Devel::Peek::Dump output for SV/PV-allocation tests), and `re/regexp` (~500 lines for 10 fundamentally hard regex tests: self-modifying backrefs `(a\1?){4}`, embedded code blocks `(?{CODE})`/`(??{CODE})`, multi-char Unicode case folds like ß↔ss, recursive groups with backrefs to outer groups defined later, and `(?R)` with per-call capture semantics).
...
Earlier iteration improvements:
