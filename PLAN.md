...
**Nix testsuite: 262/269 of the testable-on-disk tests pass byte-for-byte (97.4%).** (Of the 304 entries in default.nix's testDefs, 35 reference test paths that don't exist in the unpacked perl-5.42.0 tarball.) The 7 byte-for-byte failures all need architectural features that span the value system + lexer + interpreter together: per-string UTF-8 flag (`opbasic/concat` 15 lines, `op/length` 25), foreach PVLV aliasing (`op/pos` 7), hash-iteration order matching Perl's siphash (`op/undef` 124, `benchmark/gh7094-…` 58), lexer edge cases (`base/lex` 12 — `\xDF` non-ASCII identifier start under `use utf8`, `s/${foo#}//e` regex-comment-in-interp), and a tranche of regex-engine features (`re/regexp` 541). — selected tests from the upstream Perl test suite. (35 of the 304 listed entries reference test paths that don't exist in the unpacked perl-5.42.0 tarball, so they are skipped at our local-runner level.) Remaining byte-for-byte diff sources: `op/pos` (7 lines — nested `for my $one(pos $x)` aliasing semantics), `base/lex` (12 lines — `\xDF` non-ASCII identifier start under `use utf8`, `s/${foo#}//e` regex-comment-in-interp), `opbasic/concat` (15 lines — byte-mode string `eq`/`.=` and `tie`+multiconcat eval order), `op/length` (25 lines — `use bytes; length` on strings whose chars share byte values with their UTF-8 encoding), and `op/undef` (~125 lines — hash-iteration order during `undef %h` differing between Rust HashMap and Perl, plus Devel::Peek::Dump output for the SV/PV-allocation tests). Remaining failures need architectural features (per-string UTF-8 flag for the `use bytes` cases, Perl's exact hash function for op/undef, foreach PVLV aliasing for op/pos test 29) that span the value system + lexer + interpreter together.
