...
**Nix testsuite: 266/269 of the testable-on-disk tests pass byte-for-byte (98.9%).** (Of the 304 entries in default.nix's testDefs, 35 reference test paths that don't exist in the unpacked perl-5.42.0 tarball.) Only 2 hard byte-for-byte failures remain (plus the intermittent `benchmark/gh7094-…` whose diff is stdout-buffering vs survey-script timeout, not a real defect): `op/undef` (~100-150 lines depending on hash-seed luck — hash-iteration order during `undef %h` plus Devel::Peek::Dump output for SV/PV-allocation tests), and `re/regexp` (~32 lines for 4 hard regex tests: self-modifying backrefs `(a\1?){4}` and embedded `(?{CODE})` inside a quantified group that should fire once per iteration — plus 4 lines of (eval N) sequence number mismatch in the `\N{NAME}` die path).
