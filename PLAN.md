...
- **POSIX class `[:NAME:]` validation inside regex character classes**: `validate_regex_pattern` now (a) skips embedded `[:…:]` atomically when scanning for the outer `]`, and (b) rejects names not in the canonical 14-class set with `POSIX class [:NAME:] unknown`. The Rust regex crate previously accepted `[[:xyz:]]` as a literal class — silent semantic divergence — so this also patches the wrong matches.
